...
export const article = {
  ...
  description: "Property-Based Testing with `hypothesis`, and associated use cases",
};
...
Another testing paradigm similar to property-based testing is *fuzz testing*,
...
There's other tactics, like metamorphic testing or hyperproperties, that I
won't get into here (because I honestly don't understand them yet), but one
great resource besides the `hypothesis` documentation and the blogs / talks
given by the `hypothesis` core devs is [Hillel Wayne's
...
-   `{8,}` matches if and only if the length of the sample text is greater than
